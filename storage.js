...
const getEventById = (eventId) => firebase.database().ref('events/'+eventId).once('value').then(snapshot => {
    return snapshot.val();
});
...
const isUserAlreadyVoted = (eventId, userId) => {
    return firebase.database().ref('events/'+eventId+'/vote/'+userId).once('value').then(snapshot => {
...
const getActiveEventTitle = () => {
    return getActiveEventId().then(eventId => {
        if (eventId) {
...
const getEventVotes = (eventId) => {
    return firebase.database().ref('events/'+eventId+'/vote').once('value').then(snapshot => {
        const votes = [];
        snapshot.forEach(x => {
            votes.push(x.val());
        });
